...
async def whatsapp_send_otp(request: WhatsAppOTPRequest):
    ...
    # Check if user exists in patients or users collection
    ...
    # Send OTP via WhatsApp service (it generates + stores + sends the OTP)
    result = await send_whatsapp_otp(phone, "patient_signup")
    
    if result.get("success") and not result.get("mock"):
        # WhatsApp OTP sent successfully — store flow metadata for verification step
...
async def whatsapp_verify_otp(request: WhatsAppOTPVerify):
    ...
    # Get flow metadata stored during send-otp
    flow_key = f"whatsapp_{phone}"
    flow_data = patient_otp_storage.get(flow_key, {})
    flow_type = flow_data.get("flow_type", "signup")
    existing_user = flow_data.get("existing_user")
    
    # Verify OTP via the whatsapp_otp service (single source of truth)
    ...
    # Clean up flow data
    if flow_key in patient_otp_storage:
        del patient_otp_storage[flow_key]
    ...
    # For existing users, generate JWT directly after OTP verification
    ...
    # For new users or users needing password, return verification_token for further steps
    return {
        "success": True,
        "verified": True,
        "phone": phone,
