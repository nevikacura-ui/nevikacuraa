...
async def whatsapp_send_otp(request: WhatsAppOTPRequest):
    ...
    # Try to send real WhatsApp OTP using the service
    # The service generates and stores its own OTP
    result = await send_whatsapp_otp(phone, "patient_signup")
    
    if result.get("success"):
        # Get the OTP that was generated by the service
        otp_from_service = result.get("otp")  # Available in mock mode
        
        # Store flow information for verification step
        # Use the same key format that the service uses for consistency
        ...
        # Also store in patient_otp_storage for local verification backup
        if otp_from_service:
            patient_otp_storage[f"whatsapp_{phone}"] = {
                "otp": otp_from_service,
                "type": "whatsapp",
                "identifier": phone,
                "flow_type": flow_type,
                "existing_user": existing,
        ...
        if not result.get("mock"):
            # WhatsApp OTP sent successfully via MSG91
            return {
                "success": True,
            ...
            # WhatsApp failed but we have mock OTP - still allow for dev/testing
            # In production, redirect to email
            logger.warning(f"WhatsApp OTP in mock mode for {phone[-4:]}")
            return {
                "success": True,
                "message": "OTP sent via WhatsApp",
                "phone": phone,
        ...
        # WhatsApp completely failed - redirect to email verification
        return {
            "success": False,
