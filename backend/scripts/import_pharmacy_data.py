"""
Fast Import: Orange Pharmacy medicines from OneDrive Excel.
Uses Python-side dedup + bulk insert (no regex). 
"""
...
XLSX_PATH = "/tmp/orange_pharmacy_data.xlsx"
BATCH_SIZE = 10000
...
# Build name lookup (lowercase) for existing medicines
print("Indexing existing medicines...")
existing_names = {}
for doc in col.find({"store": "orange_pharmacy"}, {"_id": 0, "name": 1, "id": 1}):
...
inserts = []
skip = 0
seen = set(existing_names.keys())  # Track all names to prevent dupes
...
for i, row in enumerate(ws.iter_rows(min_row=2, values_only=True)):
    ...
    if name_lower in existing_names:
        # Update images for existing medicine
        if image_url:
        ...
        inserts.append(InsertOne({
            ...
            "category": "General", "unit": "unit", "form": "Other",
...
final = col.count_documents({"store": "orange_pharmacy"})
print(f"\n=== Import Complete ===")
