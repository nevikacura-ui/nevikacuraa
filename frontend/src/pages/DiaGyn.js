...
const DiaGyn = () => {
  ...
  const getSelectedFeeAmount = () => {
    return CONSULTATION_FEES[selectedFee]?.amount || 0;
  };

  const handlePayment = async () => {
    const feeAmount = getSelectedFeeAmount();
    
    // If no payment required, proceed directly to booking
    if (feeAmount === 0) {
      await handleBooking();
      return;
    }
    ...
    try {
      ...
      // Create a temporary appointment reference ID
      const tempReferenceId = `temp_${Date.now()}_${patientInfo.phone.slice(-4)}`;
      
      const response = await axios.post(`${API}/payments/create-checkout`, {
        payment_type: 'appointment',
        reference_id: tempReferenceId,
      ...
      if (response.data.success) {
        if (!response.data.payment_required) {
          // No payment needed, proceed with booking
          await handleBooking();
        } else {
