...
import {
  ArrowLeft, X, FlaskConical, CheckCircle2, Phone, Home, MapPin,
  CreditCard, Loader2
} from 'lucide-react';

/** Dedicated clean checkout view — rendered as early return when step === 4 */
const MangoCheckoutDedicated = () => {
  const m = useMango();

  return (
    <div className="min-h-screen bg-gradient-to-b from-emerald-50 via-emerald-50/50 to-white">
      {/* Header */}
      <div className="bg-gradient-to-r from-green-600 to-emerald-600 text-white p-4 sticky top-0 z-10 shadow-lg">
        <div className="max-w-lg mx-auto flex items-center gap-3">
          <button onClick={() => m.navigate('/cart')} className="p-2 hover:bg-white/20 rounded-full transition-colors" data-testid="checkout-back-btn">
            <ArrowLeft className="w-5 h-5" />
          </button>
          <FlaskConical className="w-6 h-6" />
          <h1 className="text-lg font-semibold">Nevika Labs Checkout</h1>
        </div>
      </div>
      ...
      <main className="max-w-lg mx-auto px-4 py-6 space-y-6">
        <Card className="p-5 rounded-2xl border-green-500/30 bg-green-500/5">
          <h3 className="font-medium text-[#1E293B] mb-3 flex items-center gap-2">
            <FlaskConical className="w-4 h-4 text-green-500" /> Selected Tests ({m.labCart.length})
          </h3>
          {m.labCart.length === 0 ? (
            <div className="text-center py-4">
              <p className="text-slate-500">No tests in cart</p>
              <Button onClick={() => m.navigate('/mango')} className="mt-3 bg-green-500 hover:bg-green-600">Browse Tests</Button>
            </div>
          ) : (
            <div className="space-y-2">
                ...
                <div key={test.name} className="bg-white p-3 rounded-xl border border-green-500/20">
                  ...
                  {member?.name && (
                    <div className="mt-1.5 flex items-center gap-1.5">
                      <span className="text-[11px] px-2 py-0.5 rounded-full bg-green-100 text-green-700 font-medium" data-testid={`checkout-member-${test.name.replace(/\s+/g,'-')}`}>
              ...
              <div className="flex justify-between items-center pt-3 border-t border-green-500/20 mt-2">
                ...
                <span className="font-bold text-lg text-green-600">
                  {m.labCart.reduce((s, t) => s + (t.price || 0), 0) > 0 ? `₹${m.labCart.reduce((s, t) => s + (t.price || 0), 0)}` : 'Price on confirmation'}
                </span>
              </div>
            </div>
          )}
            ...
            {m.auth.otpSent && (
              <div className="mt-4 space-y-4">
                ...
                <Button onClick={m.verifyOtp} disabled={m.otp.join('').length !== 6 || m.auth.loading}
                  className="w-full bg-gradient-to-r from-green-500 to-green-600 text-white py-5 rounded-full font-semibold" data-testid="checkout-verify-otp-btn">
        ...
        {m.verificationToken && (
          <>
            <div className="flex items-center justify-center gap-2 text-emerald-600 mb-2">
              <CheckCircle2 className="w-5 h-5" /><span className="text-sm font-medium">Phone verified: +91 {m.patientInfo.phone}</span>
            </div>
              ...
              {m.collectionType === 'home' && (
                <div className="mt-4 space-y-3">
                  ...
                  <div className="bg-green-500/5 border border-green-500/20 rounded-xl p-3 text-xs text-green-600">
                    ...
                    <ul className="mt-1 space-y-0.5"><li>• Phlebotomist calls 30 mins before</li><li>• Sample collected at doorstep</li><li>• Reports via email within 24 hours</li></ul>
                  </div>
                </div>
              )}
            ...
            <Button onClick={m.handleSubmit}
              disabled={m.loading || m.labCart.length === 0 || (m.collectionType === 'home' && !m.patientInfo.address?.trim())}
              className="w-full bg-gradient-to-r from-green-600 to-green-700 text-white py-6 rounded-full text-lg font-semibold shadow-lg" data-testid="checkout-proceed-to-payment">
              {m.loading ? (<><Loader2 className="w-5 h-5 animate-spin mr-2" /> Processing...</>) : (<><CreditCard className="w-5 h-5 mr-2" /> Continue to Payment</>)}
            </Button>
          </>
        )}
      </main>
      <div className="px-4 pb-6 space-y-4"><OrangePromoCard /></div>
    </div>
  );
};

export default MangoCheckoutDedicated;
