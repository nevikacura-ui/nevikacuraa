      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 md:py-20">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4 md:gap-6">
          {services.map((service) => (
            <div
              key={service.id}
              className={`group relative h-full ${service.tallerCard ? 'min-h-[320px]' : 'min-h-[260px]'} flex flex-col justify-between ${service.tallerCard ? 'p-6' : 'p-5'} rounded-2xl transition-all duration-300 hover:scale-[1.03] hover:shadow-2xl border ${service.accentColor} ${service.bgColor} cursor-pointer overflow-hidden ${service.alyneCard ? 'p-0' : ''}`}
                ...
                {service.useTextLogo ? (
                  ...
                  <img 
                    ...
                    className={`transition-transform group-hover:scale-105 ${
                      ...
                      service.alyneCard ? 'w-full h-full object-cover rounded-2xl' :
