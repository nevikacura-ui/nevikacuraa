            ...
            {featuredServices.map((service) => (
              ...
              >
                ...
                <div className="relative flex items-center p-5 sm:p-6 min-h-[140px] lg:min-h-[160px]">
                  <div className={`flex-shrink-0 mr-4 sm:mr-5 flex items-center justify-center ${service.id === 'mango' ? 'w-32 sm:w-40 lg:w-48 h-20 sm:h-24 lg:h-28 bg-white rounded-xl shadow-sm p-2' : 'w-28 sm:w-36 lg:w-44'}`}>
                    <img 
                      src={service.logo} 
                      alt={service.name} 
                      className={`object-contain transition-transform duration-300 group-hover:scale-110 ${service.containImage ? 'rounded-xl' : ''} ${service.id === 'mango' ? 'w-[85%] h-[85%]' : 'w-full h-auto'}`}
