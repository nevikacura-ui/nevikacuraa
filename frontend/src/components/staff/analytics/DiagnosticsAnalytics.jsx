  ...
  const fetchAnalytics = async () => {
    ...
    try {
      ...
      setAnalytics({
        ...
        revenue: 245000,
        revenue_change: 25.6,
        loyalty_points_given: 6200,
        status_breakdown: {
          'Test Booked': 14,
          'Sample Collected': 14,
          'In Process': 12,
          'Reports Generated': 116
        },
        test_categories: [
        ...
        popular_tests: [
          { name: 'CBC', count: 45 },
          { name: 'HbA1c', count: 32 },
          { name: 'Thyroid Profile', count: 28 },
          { name: 'Lipid Profile', count: 25 },
          { name: 'Vitamin D', count: 22 }
        ]
      });
    }
    setLoading(false);
    ...
    <div className="space-y-6" data-testid="diagnostics-analytics">
        ...
        <div className="flex gap-2">
          {['today', 'week', 'month'].map(range => (
            <Button
              ...
              className={timeRange === range ? 'bg-purple-500' : ''}
            >
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <Card className="p-4 bg-gradient-to-br from-purple-50 to-pink-50 border-purple-200">
          <div className="flex items-start justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-purple-900">{analytics?.total_tests}</p>
            </div>
        ...
        <Card className="p-4 bg-gradient-to-br from-green-50 to-emerald-50 border-green-200">
          <div className="flex items-start justify-between">
            <div>
              <p className="text-sm text-green-700">Reports Done</p>
              <p className="text-2xl font-bold text-green-900">{analytics?.reports_generated}</p>
            </div>
        ...
        <Card className="p-4 bg-gradient-to-br from-blue-50 to-cyan-50 border-blue-200">
          <div className="flex items-start justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-blue-900">{analytics?.avg_turnaround}h</p>
            </div>
        ...
        <Card className="p-4 bg-gradient-to-br from-amber-50 to-yellow-50 border-amber-200">
          <div className="flex items-start justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-amber-900">{analytics?.loyalty_points_given?.toLocaleString()}</p>
            </div>
        ...
        <Card className="p-4">
          ...
          <div className="space-y-3">
            {Object.entries(analytics?.status_breakdown || {}).map(([status, count]) => {
              const colors = {
                'Test Booked': 'bg-blue-500',
                'Sample Collected': 'bg-yellow-500',
                'In Process': 'bg-purple-500',
                'Reports Generated': 'bg-green-500'
              };
                ...
                <div key={i} className="space-y-1">
                  ...
                  <div className="h-3 bg-gray-200 rounded-full overflow-hidden">
                    <div 
                      ...
                      style={{ width: `${(cat.count / totalCategoryCount) * 100}%` }}
                    />
                  </div>
                </div>
              ))}
      ...
      <Card className="p-4 border-green-200 bg-gradient-to-r from-green-50 to-emerald-50">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm text-green-700">Revenue This {timeRange.charAt(0).toUpperCase() + timeRange.slice(1)}</p>
              <p className="text-2xl font-bold text-green-900">₹{analytics?.revenue?.toLocaleString()}</p>
            </div>
          </div>
