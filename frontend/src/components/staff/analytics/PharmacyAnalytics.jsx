  ...
  const fetchAnalytics = async () => {
    ...
    try {
      ...
      setAnalytics({
        ...
        delivery_rate: 80.9,
        pending: 12,
        avg_fulfillment_time: 45, // minutes
    ...
    <div className="space-y-6" data-testid="pharmacy-analytics">
        ...
        <div className="flex gap-2">
          {['today', 'week', 'month'].map(range => (
            <Button
              ...
              className={timeRange === range ? 'bg-orange-500' : ''}
            >
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <Card className="p-4 bg-gradient-to-br from-orange-50 to-amber-50 border-orange-200">
          <div className="flex items-start justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-orange-900">{analytics?.total_orders}</p>
            </div>
        ...
        <Card className="p-4 bg-gradient-to-br from-blue-50 to-cyan-50 border-blue-200">
          <div className="flex items-start justify-between">
            <div>
              <p className="text-sm text-blue-700">Avg Fulfillment</p>
              <p className="text-2xl font-bold text-blue-900">{analytics?.avg_fulfillment_time} min</p>
            </div>
          ...
          <div className="flex items-end gap-1 h-32">
            {analytics?.hourly_orders?.map((hour, i) => (
              <div key={i} className="flex-1 flex flex-col items-center gap-1">
                <div 
                  className="w-full bg-orange-400 rounded-t hover:bg-orange-500 transition-colors cursor-pointer"
                  style={{ height: `${(hour.orders / maxOrders) * 100}px` }}
      ...
      {analytics?.pending > 5 && (
        <Card className="p-4 border-amber-300 bg-gradient-to-r from-amber-50 to-orange-50">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="font-medium text-amber-800">{analytics?.pending} Orders Pending</p>
              <p className="text-sm text-amber-600">Consider prioritizing older orders for delivery</p>
            </div>
          </div>
        </Card>
      )}
